...
const currentCoachList = ref([]); // 真实接口返回的教练列表
...
const fetchInitData = async () => {
  try {
    ...
    if (res.code === 200 && res.data) {
      ...
      // 容错：如果是散客(无驾校)，且当前Tab在"本校教练"，自动切回"我的教练"
      if (!studentContext.schoolId && currentTab.value === 'school') {
        currentTab.value = 'my';
      }
    }
  } catch (error) {
    console.error('获取预约大厅初始化数据异常:', error);
  }
};
...
const generateDateList = () => {
  ...
  for (let i = 0; i < 7; i++) {
    ...
    const fullDate = `${targetDate.getFullYear()}-${month}-${day}`; // 格式必须是 yyyy-MM-dd
    ...
    list.push({
      ...
      dayLabel: `${month}-${day}`,
      weekLabel,
      // 这里的蓝点仅作装饰，真实业务中若要精确到每天是否有课，需要后端返回日历概览数据，目前默认显示
      hasSchedule: true 
    });
  }
  dateList.value = list;
  ...
  if (!selectedDate.value) {
    selectedDate.value = list[0].fullDate;
  }
};

// 每次进入页面时触发，确保日期和排班最新
onShow(() => {
  generateDateList(); // 跨天时保证日期栏实时更新
  fetchInitData();
  fetchCoachList();
});
...
const selectDate = (dateStr) => {
  if (selectedDate.value === dateStr) return; // 避免重复点击
  selectedDate.value = dateStr;
  fetchCoachList(); // 日期改变，重新拉取教练列表
};

const switchTab = (tab) => {
  if (currentTab.value === tab) return;
  currentTab.value = tab;
  fetchCoachList(); // Tab改变，重新拉取教练列表
};

const goToSchedule = (coachId) => {
  // 携带选中的教练ID和日期前往具体的时段选择页
  uni.navigateTo({
