<template>
  <view class="detail-container">
    ...
    <view class="scroll-content">
      <view class="subject-section base-box">
        ...
        <view class="tag-group">
          <view class="tag-item" v-for="sub in subjectList" :key="sub.id"
            :class="{ 'active': selectedSubjectId === sub.id }" @click="selectSubject(sub.id)">{{ sub.name }}</view>
        </view>

        <view class="sub-item-group" v-if="itemList.length > 0">
          <view class="sub-tag" v-for="item in itemList" :key="item.id"
            :class="{ 'active': selectedItemId === item.id }" @click="selectedItemId = item.id">{{ item.name }}</view>
        </view>
      </view>
      ...
      <view class="session-section base-box">
        <view class="section-title">
          <text>选择时段</text>
          <text class="sub-tip" v-if="coachInfo.dailyBookLimit > 1">（可多选，最多 {{ coachInfo.dailyBookLimit }} 个）</text>
        </view>
        ...
        <view class="session-grid" v-else-if="sessionList.length > 0">
          ...
          <view class="session-item" v-for="session in sessionList" :key="session.id" :class="{
            'full': session.status === 'full' || session.hasBooked,
            'selected': isSessionSelected(session.id)
          }" @click="toggleSession(session)">
            <view class="time">{{ session.startTime }} - {{ session.endTime }}</view>

            <view class="status-text booked" v-if="session.hasBooked">已预约</view>
            <view class="status-text" v-else-if="session.status === 'full'">已满</view>
    ...
    <view class="fixed-bottom-bar">
      ...
      <button class="submit-btn" :class="{ 'disabled': selectedSessions.length === 0 || !selectedSubjectId }"
...
// 获取当天被选中的时段数量
const sessionSelectedLength = computed(() => {
  return sessionList.value.filter(item => item.hasBooked).length;
});
...
const toggleSession = (session) => {
  ...
  if (session.hasBooked) {
    uni.showToast({ title: '您已预约此时段，无需重复选择', icon: 'none' });
    return;
  }
  ...
  if (index > -1) {
    ...
    // 动态限制一次最多约几个课时 默认 2
    if (selectedSessions.value.length + sessionSelectedLength.value >= coachInfo.dailyBookLimit) {
      uni.showToast({ 
        title: `该教练单日最多只能预约 ${coachInfo.dailyBookLimit} 个时段哦`, 
        icon: 'none' 
      });
      return;
    }
...
const submitAppointment = async () => {
  ...
  try {
    ...
    if (res.code === 200) {
      // 下单成功
      await uni.showToast({ title: '预约成功！', icon: 'success' });
      // 刷新当前页面的排班数据，让学员看到最新的剩余名额
      // fetchDetailData();
      // 延迟 0.8 秒后，跳转到“练车记录”页面，让学员查看这笔“待确认”的订单
      setTimeout(() => {
        uni.navigateTo({
    ...
    await uni.showToast({ title: error?.message ? error.message :'网络开小差了，请重试', icon: 'none' });
...
/* 1. 顶部教练名片 */
.coach-header {
  background: linear-gradient(to bottom, #007aff, #409eff);
  color: #fff;
  padding: 40rpx 30rpx;
